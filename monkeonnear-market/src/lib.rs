...
use near_sdk::{
    ...
    BorshStorageKey, CryptoHash, Gas, PanicOnDefault, Promise, Timestamp, promise_result_as_success
};
...
impl Contract {
    ...
    fn internal_add_market_data(
        ...
        let nft_contract: AccountId = nft_contract_id.parse().unwrap();
        ext_paras::nft_token(
        ...
        .then(ext_nft_dos::on_nft_token(
            ...
            env::current_account_id(),
            0,
            BASE_GAS_TOKEN + Gas(3_000_000_000_000),
        ));
    }

    #[private]
    pub fn on_nft_token(&mut self, 
        owner_id: AccountId,
        approval_id: u64,
        nft_contract_id: String,
        token_id: TokenId,
        ...
        let ret = near_sdk::serde_json::from_slice::<TokenData>(&result.unwrap()).expect("TokenData");
        
        let contract: AccountId = nft_contract_id.parse().unwrap();
        self.internal_add_market_data_final(
            owner_id.clone(),
            approval_id,
            contract,
            token_id.to_string(),
    ...
    fn internal_add_market_data_final(
        &mut self,
        owner_id: AccountId,
        approval_id: u64,
        nft_contract_id: AccountId,
        ...
        env::log_str(
            &json!({
                ...
                "params": {
                    ...
                    "royalty": Some(token_data.royalty.unwrap().iter().map(|(k, v)| RoyaltyFormat {account_id: AccountId::new_unchecked(k.to_string()), percent: v.to_string()}).collect::<Vec<RoyaltyFormat>>()),
                    "reference": token_data.metadata.reference,
                    "ft_token_id": ft_token_id,
                    "price": price,
    ...
    #[payable]
    pub fn delete_market_data_contract(&mut self, nft_contract_id: AccountId, owner_id: AccountId) {
        assert_one_yocto();
        self.assert_owner();
        let market: Vec<MarketDataV1> = self.market.iter()
                                        .filter(|(_k, v)| v.nft_contract_id == nft_contract_id && v.owner_id == owner_id)
                                        .map(|(_k, v)| MarketDataV1 {
                                            owner_id: v.owner_id,
                                            approval_id: v.approval_id,
                                            nft_contract_id: v.nft_contract_id,
                                            token_id: v.token_id,
                                            ft_token_id: v.ft_token_id,
                                            price: v.price,
                                        }).collect();

        for i in 0..market.len() {
            self.internal_delete_market_data(&market[i].nft_contract_id, &market[i].token_id);
        }
    }
    ...
    pub fn get_market(self,
        ...
        assert!(
            (market.len() as u128) > start_index,
